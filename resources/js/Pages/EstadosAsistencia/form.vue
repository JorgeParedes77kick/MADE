<script setup>
  ...
  const props = defineProps({
    action: String,
    estAsistencia: { type: Object, default: {} },
    status: String,
  });
  ...
  const submit = async (form) => {
    ...
    const routeName = `estados-asistencia.${action}`;
    ...
    try {
      ...
      if (response?.data?.message) {
        ...
        router.visit(route('estados-asistencia.index'));
      }
    } catch (err) {
  ...
  <MainLayout>
    <v-container>
      <v-card color="background" class="" :disabled="loading" :loading="loading">
        ...
        <v-card-title>
          <ButtonBack :href="route('estados-asistencia.index')" /> ESTADOS DE ASISTENCIA
          {{ CRUD.create !== action ? `#${inputForm.id}` : '' }}
        </v-card-title>
        <v-card-subtitle>{{ ACCION[action] }} de Estado de Asistencia</v-card-subtitle>
        <v-card-text>
