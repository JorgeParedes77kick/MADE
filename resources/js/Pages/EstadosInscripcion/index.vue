<script setup>
  ...
  const onClickDelete = async (item) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(route('estados-inscripcion.destroy', item.id));
  ...
  <MainLayout>
    <v-container>
      <v-card color="background" class="px-4 py-2">
        <v-card-title>ESTADOS DE INSCRIPCIÓN </v-card-title>
        <div>
          <v-row>
            <v-col class="d-flex justify-end">
              <Link :href="route('estados-inscripcion.create')">
                <v-btn :to="{ name: 'estados-inscripcion.create' }" color="success" class="ms-auto">
                  Crear Nuevo Estado de Inscripción
                </v-btn>
              </Link>
            </v-col>
          </v-row>
              ...
              >
                ...
                ><template v-slot:[`item.acciones`]="{ item }">
                  <div class="d-flex inline-flex ga-2">
                    <Link :href="route('estados-inscripcion.show', item)">
                      <v-btn as="v-btn" color="info" small> Ver </v-btn>
                    </Link>
                    <Link :href="route('estados-inscripcion.edit', item)">
                      <v-btn
                        :to="{ name: 'estados-inscripcion.edit', params: { id: item.idCrypt } }"
                        color="secondary"
                        small
                      >
                        Editar
                    ...
                    <v-btn v-if="item.id > 6" color="error" small @click="onClickDelete(item)"
                      >Eliminar
                    </v-btn>
                  </div>
                </template>
